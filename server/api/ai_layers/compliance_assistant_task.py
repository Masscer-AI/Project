"""
Celery task for compliance assistant (KYB e-sign) conversations.
...
def compliance_assistant_task(
    ...
    try:
        ...
        emit_event("error", {"error": f"Compliance assistant '{agent_slug}' not found"})
        from api.ai_layers.agent_task_helpers import clear_agent_task_active

        clear_agent_task_active(conversation_id)
        return {"status": "error", "error": "Compliance assistant not found"}

    organization = agent.organization or conversation.organization
    if not organization:
        emit_event("error", {"error": "No organization context for compliance assistant"})
        from api.ai_layers.agent_task_helpers import clear_agent_task_active
    ...
    try:
        ...
        inputs_data = AgentSessionInputs(
            ...
            tool_names=compliance_tool_names,
            plugin_slugs=[],
        ...
        session = AgentSession.objects.create(
            ...
            task_type="compliance_assistant",
            inputs=inputs_data,
            outputs={},
            agent_index=0,
        )
        ...
        tools = resolve_compliance_tools(
        ...
        if not conversation.title:
            logger.info(
                "compliance_assistant_task generate_title: conversation_id=%s agent_slug=%s",
                conversation_id,
                agent.slug,
            )
        ...
        logger.error(
            "compliance_assistant_task failed: conversation=%s error=%s",
            conversation_id,
            str(e),
        )
        emit_event("error", {"error": str(e)})
