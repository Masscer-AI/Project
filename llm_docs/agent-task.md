...
- **WhatsApp:** Meta webhook → `async_handle_webhook` → `whatsapp_conversation_agent_task` → `conversation_agent_task` (inline), then `deliver_whatsapp_reply` sends **file attachments** linked to the assistant message (`create_image`, `create_speech`, `generate_video`, `render_document_template`) via Graph media messages (public `link` when `API_BASE_URL`/S3 URL is HTTPS, else upload to `/media`), then sends the assistant text. WAMIDs are stored on `Message.metadata` (`whatsapp_media_wamids`, `whatsapp_wamid`). Threads are `messaging.Conversation` rows linked by `ws_number` + `whatsapp_user_number`; **`Conversation.user` is always null** (anonymous visitor, like chat widgets); org comes from the line for billing. Tools come from `WSNumber.capabilities` (subset of internal tools — no `read_plugin_instructions` or org-only tools; see `api/whatsapp/capability_tools.py`), and template tools (`list_document_templates`, `render_document_template`) are auto-injected when the assigned agent has enabled template assignments. **`create_image` / `create_speech` do not require org flags `image-tools` or `chat-generate-speech`** when the conversation has a `ws_number` (same bypass as `chat_widget_id`; see `api/ai_layers/tools/embedded_channels.py`). POST webhooks verify `X-Hub-Signature-256` when `WHATSAPP_APP_SECRET` is set; inbound WAMIDs are de-duplicated via `Message.metadata.whatsapp_inbound_wamid` before enqueueing the agent. Agent-task events route on `whatsapp:<conversation_uuid>` (same idea as `widget_session:…`). In Django admin, open a **WS number** and use **Register phone number**, **Setup WABA & webhook**, or **Check webhook config** (requires public API origin `API_BASE_URL` or `API_URL`, plus `WHATSAPP_GRAPH_API_TOKEN`, `WHATSAPP_APP_SECRET`, and `WHATSAPP_WEBHOOK_VERIFY_TOKEN`).
