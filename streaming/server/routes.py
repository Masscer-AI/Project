...
async def get_oauth_spa(full_path: str):
    """SPA shell for OAuth consent flow."""
    file_path = os.path.join("client", "dist", "index.html")
    if os.path.exists(file_path):
        with open(file_path, "r") as file:
            html_content = file.read()
...
@router.get("/esign/{full_path:path}", response_class=HTMLResponse)
async def get_esign_spa(full_path: str):
    """SPA shell for the public Mifiel signing page (/esign/sign/<id>)."""
    file_path = os.path.join("client", "dist", "index.html")
    if os.path.exists(file_path):
        with open(file_path, "r") as file:
            html_content = file.read()
